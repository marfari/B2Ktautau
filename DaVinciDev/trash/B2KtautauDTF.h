 ...
 * -  Example:
 ...
 from Configurables import B2KtautauDTF
 tuple.B.ToolList +=  [ "B2KtautauDTF/Fit",            # just a refit
 "B2KtautauDTF/MassFit",        # fit with J/psi mass constraint
 "B2KtautauDTF/PVFit" ]         # fit with all constraints I can think of

 tuple.B.addTool(B2KtautauDTF("MassFit"))
 tuple.B.MassFit.constrainToOriginVertex = False
 tuple.B.MassFit.daughtersToConstrain = [ "J/psi(1S)" ]

 tuple.B.addTool(B2KtautauDTF("PVFit"))
 ...
 tuple.B.ToolList +=  ["B2KtautauDTF/SubLambda", "B2KtautauDTF/SubLambdabar" ]
 subDTF = B2KtautauDTF("SubLambda", Verbose=True,
...
struct IDVAlgorithm;
/** @class B2KtautauDTF B2KtautauDTF.h
 *
 * Yasmine Amhis and Matt Needham
 * 30-10-10
 */
class B2KtautauDTF : public TupleToolBase, virtual public IParticleTupleTool {
...
public:
  ...
  B2KtautauDTF( const std::string& type, const std::string& name, const IInterface* parent );

  ~B2KtautauDTF() = default; ///< Destructor
...
private:
  ...
  StatusCode fit( DecayTreeFitter::Fitter& fitter, const LHCb::Particle* P, const LHCb::VertexBase* pv,
                  const std::string& prefix, TupleMap& tMap, Tuples::Tuple& tuple ) const;

  //Added by AV
  StatusCode fillVtxIter(const DecayTreeFitter::Fitter& fitter, const std::string& prefix, Tuples::Tuple& tuple, std::vector<Gaudi::XYZPoint> BV, std::vector<Gaudi::XYZPoint> DV1, std::vector<Gaudi::XYZPoint> DV2, std::vector<float> B_M) const;

  //Added by AV
  StatusCode fillChi2Iter(DecayTreeFitter::Fitter& fitter, std::vector<float> chisq_iters, const std::string& prefix, Tuples::Tuple& tuple ) const;
  ...
  Gaudi::XYZVector makeTransformation_vec(Gaudi::XYZVector p_K, Gaudi::XYZPoint refPoint, Gaudi::XYZVector theVector, bool invFlag);
  
  Gaudi::XYZPoint makeTransformation_point(Gaudi::XYZVector p_K, Gaudi::XYZPoint refPoint, Gaudi::XYZPoint thePoint, bool invFlag);
