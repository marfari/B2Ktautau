...
namespace DecayTreeFitter {
  ...
  // ==========================================================================
  Fitter::Fitter( const LHCb::Particle& bc, const bool forceFitAll, const ITrackStateProvider* extrapolator, const bool useTrackTraj )
      : m_particle( &bc )
      , m_decaychain( 0 )
      , m_fitparams( 0 )
      , m_status( UnFitted )
  ...
  // constructor from the particle (decay head)
  // ==========================================================================
  Fitter::Fitter( const LHCb::Particle& bc, const ITrackStateProvider* extrapolator, const bool forceFitAll, const bool useTrackTraj )
      : m_particle( &bc )
      , m_decaychain( 0 )
      , m_fitparams( 0 )
      , m_status( UnFitted )
  ...
  void Fitter::fit( int nitermax, double reldChisqConv, int maxndiverging, double dChisqQuit, std::vector<float>& chisq_iters,
    ...
    // Get taus 
    // Here taup = {+,-} for {K+,K-} and taum = {-,+} for {K-,K+} to match with root branch naming
    // In B2KTauTauDTF.cpp, taup = - always and taum = + always. It doesn't matter for the calculation
