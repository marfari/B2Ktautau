...
namespace DecayTreeFitter {
  ...
  bool Constraint::operator<( const Constraint& rhs ) const {
    // the simple way
    return m_type < rhs.m_type || ( m_type == rhs.m_type && m_depth < rhs.m_depth );
  ...
  ErrCode Constraint::filter( FitParams& fitpar ) const {
    ...
    if ( m_type <= Constraint::unknown || m_type >= Constraint::ntypes ) {
      std::cout << "Constraint: unknown constraint: " << m_type << std::endl;
      status |= ErrCode::badsetup;
    } else if ( m_type != merged && !m_node ) {
      std::cout << "Constraint: filter constraint without a node" << std::endl;
      status |= ErrCode::badsetup;
    } else {
      if ( vtxverbose >= 3 ) {
        std::cout << "filtering ";
  ...
  ErrCode Constraint::filter( FitParams& fitpar, const FitParams& reference ) const {
    ...
    if ( status.failure() && vtxverbose >= 1 )
      std::cout << "error filtering constraint: " << name() << " " << status << std::endl;

    return status;
  }

  void Constraint::print( std::ostream& os ) const {
    os << "node index = " << m_node->index() << " name = " << m_node->name().c_str() << " constraint type = " << m_type
  ...
  std::string Constraint::name() const {
    ...
    case merged:
      rc = "merged";
      break;
    case convertedphotonposition:
      rc = "convertedphotonposition";
      break;
    case externalmomentum:
      rc = "externalmomentum";
      break;
    case ntypes:
    case unknown:
