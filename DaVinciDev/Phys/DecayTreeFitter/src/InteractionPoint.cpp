...
namespace DecayTreeFitter {
  ...
  InteractionPoint::InteractionPoint( const LHCb::VertexBase& ipvertex, const LHCb::Particle& daughter,
    ...
    if ( vtxverbose >= 2 ) {
      std::cout << "InteractionPoint: initial beam spot = (" << m_ipPos( 0 ) << "," << m_ipPos( 1 ) << ","
                << m_ipPos( 2 ) << ")" << std::endl;
  ...
  ErrCode InteractionPoint::initPar1( FitParams* fitparams ) {
    ...
    for ( int row = 1; row <= 3; ++row ) fitparams->par()( posindex + row ) = m_ipPos( row - 1 );
    for ( daucontainer::iterator idau = daughters().begin(); idau != daughters().end(); ++idau ) {
  ...
  ErrCode InteractionPoint::projectIPConstraint( const FitParams& fitparams, Projection& p ) const {
    ...
    for ( int row = 1; row <= maxrow; ++row ) {
      ...
      for ( int col = 1; col <= row; ++col ) p.Vfast( row, col ) = m_ipCov( row - 1, col - 1 );
    }
    return ErrCode::success;
  }

  ErrCode InteractionPoint::projectConstraint( Constraint::Type type, const FitParams& fitparams,
                                               Projection& p ) const {
    ErrCode status;
    switch ( type ) {
  ...
  double InteractionPoint::chiSquare( const FitParams* fitparams ) const {
    ...
    for ( int row = 0; row < 3; ++row ) residual( row ) -= fitparams->par()( posindex + row + 1 );
