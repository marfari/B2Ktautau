...
namespace DecayTreeFitter {
  ...
  RecoPhoton::RecoPhoton( const LHCb::Particle& bc, const ParticleBase* mother ) : RecoParticle( bc, mother ) {
    updCache();
  }

  RecoPhoton::~RecoPhoton() {}

  ErrCode RecoPhoton::initParPhoton( FitParams* fitparams, const Gaudi::XYZPoint& motherpos ) const {
    Gaudi::XYZVector dx( m_m( 0 ) - motherpos.x(), m_m( 1 ) - motherpos.y(), m_z - motherpos.z() );
    double           l = dx.r();
  ...
  ErrCode RecoPhoton::initPar1( FitParams* fitparams ) {
  ...
  ErrCode RecoPhoton::initCov( FitParams* fitparams ) const {
    ...
    for ( int row = 1; row <= 3; ++row ) fitparams->cov().fast( momindex + row, momindex + row ) = factor * varEnergy;
    return ErrCode();
  }
  ...
  ErrCode RecoPhoton::projectRecoConstraint( const FitParams& fitparams, Projection& p ) const {
    ...
    for ( int irow = 1; irow <= 3; ++irow )
      for ( int icol = 1; icol <= irow; ++icol ) p.Vfast( irow, icol ) = m_V( irow - 1, icol - 1 );
