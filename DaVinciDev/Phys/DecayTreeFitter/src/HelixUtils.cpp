...
namespace DecayTreeFitter {
  ...
  void VtkHelixUtils::vertexFromHelix( const CLHEP::HepVector& helixpar, const BField& fieldmap,
  ...
  void VtkHelixUtils::helixFromVertex( const CLHEP::HepVector& vertexpar, int charge, const BField& fieldmap,
    ...
    if ( deltaphi > Gaudi::Units::pi )
      deltaphi -= Gaudi::Units::twopi;
    else if ( deltaphi < -Gaudi::Units::pi )
      deltaphi += Gaudi::Units::twopi;
    ...
    if ( jacobian.num_col() == 6 && jacobian.num_row() == 6 ) {
      for ( int row = 0; row < 6; ++row )
        for ( int col = 0; col < 6; ++col ) jacobian[row][col] = 0;
  ...
  void VtkHelixUtils::printHelixPar( const CLHEP::HepVector& helixpar ) {
    for ( int i = 0; i < 6; ++i ) cout << helixParName( i ).c_str() << helixpar[i] << endl;
  }

  void VtkHelixUtils::printVertexPar( const CLHEP::HepVector& vertexpar, int charge ) {
    for ( int i = 0; i < 6; ++i ) cout << vertexParName( i ).c_str() << vertexpar[i] << endl;
    cout << "charge:    " << charge << endl;
  }

  void VtkHelixUtils::helixFromVertexNumerical( const CLHEP::HepVector& vertexpar, int charge, const BField& fieldmap,
                                                CLHEP::HepVector& helixpar, CLHEP::HepMatrix& jacobian ) {
    ...
    for ( int jin = 0; jin < 6; ++jin ) {
      ...
      for ( int iex = 0; iex < 6; ++iex ) jacobian[iex][jin] = ( helixpartmp[iex] - helixpar[iex] ) / delta;
    }
  }
  ...
  double VtkHelixUtils::phidomain( const double phi ) {
    ...
    if ( phi < -Gaudi::Units::pi )
      rc += Gaudi::Units::twopi;
    else if ( phi > Gaudi::Units::pi )
      rc -= Gaudi::Units::twopi;
    return rc;
  }

  double VtkHelixUtils::helixPoca( const CLHEP::HepVector& helixpar1, const CLHEP::HepVector& helixpar2, double& flt1,
                                   double& flt2, Gaudi::XYZPoint& vertex, bool parallel ) {
  ...
  double VtkHelixUtils::helixPoca( const CLHEP::HepVector& helixpar, const Gaudi::XYZPoint& point, double& flt ) {
    double d0     = helixpar[ex_d0];
    double phi0   = helixpar[ex_phi0];
    double omega  = helixpar[ex_omega];
    double z0     = helixpar[ex_z0];
