...
namespace DecayTreeFitter {
  ...
  public:
    ...
    Projection( int dimP, int dimC )
        : m_matrixH( dimC, dimP, 0 ), m_r( dimC, 0 ), m_matrixV( dimC, 0 ), m_offset( 0 ), m_particle( 0 ) {}
    ...
    double& V( const int row, const int col ) { return m_matrixV( m_offset + row, m_offset + col ); }
    double& Vfast( const int row, const int col ) {
#ifdef VTK_BOUNDSCHECKING
      assert( m_offset + row > 0 && m_offset + col > 0 && m_offset + row <= m_matrixV.num_row() &&
