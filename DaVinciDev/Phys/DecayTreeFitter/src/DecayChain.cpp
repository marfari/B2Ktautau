...
namespace DecayTreeFitter {
  ...
  void DecayChain::printConstraints( std::ostream& os ) const {
    ...
    for ( ParticleBase::constraintlist::const_iterator it = m_constraintlist.begin(); it != m_constraintlist.end();
          ++it )
      it->print( os );
  }
  ...
  void DecayChain::initConstraintList() {
    ...
    if ( false ) {
      ...
      for ( ParticleBase::constraintlist::iterator it = m_constraintlist.begin(); it != m_constraintlist.end(); ++it ) {
        if ( it->isLineair() )
          m_mergedconstraintlist.push_back( &( *it ) );
        else
          m_mergedconstraint.push_back( &( *it ) );
      }
  ...
  double DecayChain::chiSquare( const FitParams* par ) const { return m_mother->chiSquare( par ); }
  ...
  void DecayChain::setMassConstraint( const LHCb::Particle& bc, bool add ) {
  ...
  void DecayChain::setMassConstraint( const LHCb::Particle& bc, double mass ) {
    ParticleBase* part = const_cast<ParticleBase*>( locate( bc ) );
    if ( part ) {
  ...
  void DecayChain::setMassConstraint( const LHCb::ParticleID& pid, bool add ) {
    ParticleBase::ParticleContainer particles;
    m_mother->locate( pid, particles );
    bool changed( false );
    for ( ParticleBase::ParticleContainer::iterator it = particles.begin(); it != particles.end(); ++it ) {
  ...
  void DecayChain::setMassConstraint( const LHCb::ParticleID& pid, double mass ) {
    ParticleBase::ParticleContainer particles;
    m_mother->locate( pid, particles );
    bool changed( false );
    for ( ParticleBase::ParticleContainer::iterator it = particles.begin(); it != particles.end(); ++it ) {
      ( *it )->setMassConstraint( mass );
      changed = true;
    }
    //
    if ( changed ) {
  ...
  int DecayChain::posIndex( const LHCb::Particle& bc ) const {
    const ParticleBase* base = locate( bc );
    return base ? base->posIndex() : -1;
  }

  int DecayChain::momIndex( const LHCb::Particle& bc ) const {
    const ParticleBase* base = locate( bc );
    return base ? base->momIndex() : -1;
  }

  int DecayChain::lenIndex( const LHCb::Particle& bc ) const {
    const ParticleBase* base = locate( bc );
    return base ? base->lenIndex() : 0;
  }

  ChiSquare DecayChain::chiSquare( const LHCb::Particle& bc, const FitParams& fitpars ) const {
    const ParticleBase* base = locate( bc );
    ChiSquare           chisq;
    if ( base ) {
      ...
      if ( base->mother() && base->posIndex() >= 0 && base->posIndex() != base->mother()->posIndex() )
        chisq += ChiSquare( 0, 1 );
    }
    return chisq;
  }
