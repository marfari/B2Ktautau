...
namespace DecayTreeFitter {
  ...
  void FitParams::resetPar() {
    for ( int row = 1; row <= m_dim; ++row ) m_par( row ) = 0;
  }

  void FitParams::resetCov( double scale ) {
    for ( int row = 1; row <= m_dim; ++row ) {
      for ( int col = 1; col < row; ++col ) m_cov.fast( row, col ) = 0;
      m_cov.fast( row, row ) *= scale;
      if ( m_cov.fast( row, row ) < 0 ) m_cov.fast( row, row ) *= -1;
    }
    m_chiSquare    = 0;
    m_nConstraints = 0;
    for ( int row = 1; row <= m_dim; ++row ) nConstraintsVec( row ) = 0;
    m_chiSquareMap.clear();
  }

  bool FitParams::testCov() const {
    bool okay = true;
    for ( int row = 1; row <= m_dim && okay; ++row ) okay = m_cov.fast( row, row ) > 0;
    return okay;
  }
  ...
  void FitParams::print() const {
    ...
    for ( int row = 1; row <= m_dim; ++row )
      std::cout << std::setw( 3 ) << row - 1 << std::setw( 15 ) << m_par( row ) << std::setw( 15 )
                << sqrt( m_cov( row, row ) ) << std::endl;
  ...
  CLHEP::HepSymMatrix FitParams::cov( const std::vector<int>& indexVec ) const {
    ...
    for ( int row = 1; row <= nrow; ++row )
      for ( int col = 1; col <= row; ++col ) thecov( row, col ) = m_cov( indexVec[row - 1] + 1, indexVec[col - 1] + 1 );
    return thecov;
  }
  ...
  CLHEP::HepVector FitParams::par( const std::vector<int>& indexVec ) const {
    ...
    for ( int row = 1; row <= nrow; ++row ) thepar( row ) = m_par( indexVec[row - 1] + 1 );
    return thepar;
  }
