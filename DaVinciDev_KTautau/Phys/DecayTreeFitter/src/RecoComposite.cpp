...
namespace DecayTreeFitter {
  ...
  CLHEP::HepSymMatrix convertToHepSymMatrix( const Matrix& M ) {
    ...
    for ( size_t irow = 0; irow < dim; ++irow )
      for ( size_t icol = 0; icol <= irow; ++icol ) rc.fast( irow + 1, icol + 1 ) = M( irow, icol );
    return rc;
  }
  ...
  void RecoComposite::updCache() {
    ...
    if ( vtxverbose >= 4 ) {
      std::cout << "cov matrix of external candidate: " << name().c_str() << " " << dimM() << " " << m_matrixV
                << std::endl;
    }
  }
  ...
  ErrCode RecoComposite::projectRecoComposite( const FitParams& fitparams, Projection& p ) const {
    ...
    for ( int row = 1; row <= dimM(); ++row ) {
      ...
      for ( int col = 1; col <= row; ++col ) p.Vfast( row, col ) = m_matrixV.fast( row, col );
    }
