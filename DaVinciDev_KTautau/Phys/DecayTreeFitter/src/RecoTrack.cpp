...
namespace {
  void applyBremCorrection( LHCb::State& state, double bremEnergy, double bremEnergyCov ) {
    ...
    for ( unsigned int i = 0; i < 4; ++i ) state.covariance()( 4, i ) *= dqopnewdqop;
...
namespace DecayTreeFitter {
  ...
  RecoTrack::RecoTrack( const LHCb::Particle& cand, const ParticleBase* mother, const Configuration& config )
  ...
  ErrCode RecoTrack::initCov( FitParams* fitparams ) const {
    ...
    /**
     * In case we fit a VELO track, the initial values of the
     * covariance matrix are too large.
     */
    if ( m_track->type() == LHCb::Track::Types::Velo ) {
      for ( int row = 1; row <= 3; ++row ) {
        fitparams->cov()( momindex + row, momindex + row ) =
            std::min( m_maxCovVeloTrack, 1000 * covP( row - 1, row - 1 ) );
      }
    } else {
      for ( int row = 1; row <= 3; ++row ) {
  ...
  ErrCode RecoTrack::updCache( const FitParams& fitparams ) {
    ErrCode rc;
  ...
  const LHCb::Trajectory* RecoTrack::trajectory() const { return tracktraj(); }

  CLHEP::HepVector symdiag( const CLHEP::HepSymMatrix& m ) {
    CLHEP::HepVector rc( m.num_row() );
    for ( int i = 1; i <= m.num_row(); ++i ) rc( i ) = std::sqrt( m.fast( i, i ) );
    return rc;
  }
  ...
  ErrCode RecoTrack::projectRecoConstraint( const FitParams& fitparams, Projection& p ) const {
    ...
    for ( int row = 1; row <= 5; ++row )
      for ( int col = 1; col <= row; ++col ) p.Vfast( row, col ) = m_V( row - 1, col - 1 );
