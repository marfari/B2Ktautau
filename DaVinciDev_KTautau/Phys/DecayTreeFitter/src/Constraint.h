...
namespace DecayTreeFitter {
  ...
  public:
    ...
    Constraint( const ParticleBase* node, Type type, int depth, unsigned int dim,
                int maxniter = 1 /*, double precision=1e-5*/ )
        : m_node( node ), m_depth( depth ), m_type( type ), m_dim( dim ), m_weight( 1 ), m_maxNIter( maxniter ) {}
  ...
  protected:
    Constraint( Constraint::Type type )
        : m_node( 0 ), m_depth( 0 ), m_type( type ), m_dim( 0 ), m_weight( 0 ), m_maxNIter( 0 ) {}
