...
xu_symbols = sp.symbols('BVx BVy BVz pBx pBy pBz mB ptau1x ptau1y ptau1z ptau2x ptau2y ptau2z')
lb_symbols = sp.symbols('lb0 lb1 lb2 lb3 lb4 lb5 lb6 lb7 lb8 lb9 lb10 lb11 lb12 lb13')
x_symbols = sp.symbols('PVx PVy PVz DV1x DV1y DV1z p3pi1x p3pi1y p3pi1z E3pi1 DV2x DV2y DV2z p3pi2x p3pi2y p3pi2z E3pi2 RPx RPy pKx pKy pKz BVx BVy BVz pBx pBy pBz mB ptau1x ptau1y ptau1z ptau2x ptau2y ptau2z lb0 lb1 lb2 lb3 lb4 lb5 lb6 lb7 lb8 lb9 lb10 lb11 lb12 lb13')
...
def lagrangian(params):
    ...
    ptau1y = xu_symbols[8]
    ptau1z = xu_symbols[9]
    ptau2x = xu_symbols[10]
    ptau2y = xu_symbols[11]
    ptau2z = xu_symbols[12]

    # 10 constraints applied by parameter substitution and 14 constraints applied by lagrange multipliers
    def E(m, px, py, pz):
        return (m**2 + px**2 + py**2 + pz**2)**(0.5)
    ...
    # Energy conservation in DV1 (Etau1 - E3pi1 - Enu1 = 0)
    g.append( E(mtau,ptau1x,ptau1y,ptau1z) - E3pi1 - E(mnu,ptau1x-p3pi1x,ptau1y-p3pi1y,ptau1z-p3pi1z) )
    # Energy conservation in DV2 (Etau2 - E3pi1 - Enu2 = 0)
    g.append( E(mtau,ptau2x,ptau2y,ptau2z) - E3pi2 - E(mnu,ptau2x-p3pi2x,ptau2y-p3pi2y,ptau2z-p3pi2z) )
    # 4-momentum conservation in BV
    g.append( pBx - ptau1x - ptau2x - pKx )
    g.append( pBy - ptau1y - ptau2y - pKy )
    g.append( pBz - ptau1z - ptau2z - pKz )
    g.append( E(mB,pBx,pBy,pBz) - E(mtau,ptau1x,ptau1y,ptau1z) - E(mtau,ptau2x,ptau2y,ptau2z) - E(mkaon,pKx,pKy,pKz) ) # EB - Etau1 - Etau2 - EK
...
def x_initial_estimate(init, BV, species, params):
    ...
    elif(init == 13): # TRUE init
        BV = ROOT.Math.XYZPoint( x_true[0], x_true[1], x_true[2] )
        PB = ROOT.Math.PxPyPzEVector( x_true[3], x_true[4], x_true[5], x_true[6] )
        Ptau1 = ROOT.Math.PxPyPzEVector( x_true[7], x_true[8], x_true[9], x_true[10] )
        P1 = ROOT.Math.PxPyPzEVector( x_true[11], x_true[12], x_true[13], x_true[14] )
        P2 = ROOT.Math.PxPyPzEVector( x_true[15], x_true[16], x_true[17], x_true[18] )
        P3 = ROOT.Math.PxPyPzEVector( x_true[19], x_true[20], x_true[21], x_true[22] )
        Ptau2 = ROOT.Math.PxPyPzEVector( x_true[23], x_true[24], x_true[25], x_true[26] )
        P4 = ROOT.Math.PxPyPzEVector( x_true[27], x_true[28], x_true[29], x_true[30] )
        P5 = ROOT.Math.PxPyPzEVector( x_true[31], x_true[32], x_true[33], x_true[34] )
        P6 = ROOT.Math.PxPyPzEVector( x_true[35], x_true[36], x_true[37], x_true[38] )

        pB = ROOT.Math.XYZVector( PB.x(), PB.y(), PB.z() )
        EB = PB.t()
...
def main(argv):
    ...
    file = ROOT.TFile.Open("/panfs/felician/B2Ktautau/workflow/standalone_fitter/201{0}/Species_{1}/{2}.root".format(year,species,line), "RECREATE")
    ...
    for evt in range(num_entries):
        ...
        # True initialisation
        # run_solver(11, year, species, line, BV_offline, params, max_iter=10000, eps=0.000001)
        # global firstTrial
        # firstTrial = False
        # if(status != 0):
        #     run_solver(11, year, species, line, BV_offline, params, max_iter=10000, eps=0.001)
        # if(status != 0):
        #     run_solver(11, year, species, line, BV_offline, params, max_iter=10000, eps=1)
        # if(status != 0):
        #     run_solver(11, year, species, line, BV_offline, params, max_iter=10000, eps=10)
        ...
        p3pi1 = ROOT.Math.XYZVector( x[6], x[7], x[8] )
        E3pi1 = x[9]
        p3pi2 = ROOT.Math.XYZVector( x[13], x[14], x[15] )
        E3pi2 = x[16]
        BV = ROOT.Math.XYZPoint( x[dimM], x[dimM+1], x[dimM+2] )
        pB = ROOT.Math.XYZVector( x[dimM+3], x[dimM+4], x[dimM+5] )
        mB = x[dimM+6]
        ptau1 = ROOT.Math.XYZVector( x[dimM+7], x[dimM+8], x[dimM+9] )
        ptau2 = ROOT.Math.XYZVector( x[dimM+10], x[dimM+11], x[dimM+12] )
