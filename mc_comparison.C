...
TH1D* create_histogram(RooRealVar var, TTree* t, int n, bool mc);
...
void mc_comparison(){
      ...
      TFile* fin_data = new TFile("/panfs/felician/B2Ktautau/ROOT_Data/2016/data_2016.root");
      TTree* t_data = (TTree*)fin_data->Get("ntuple/DecayTree");
      ...
      for(int i=0; i<n_var; i++){
            histos_mc.push_back(create_histogram((*ws->var(variables[i])), t_mc, n_bins, true));
            histos_data.push_back(create_histogram((*ws->var(variables[i])), t_data, n_bins, false));
      ...
      for(int i=0; i<n_var; i++){
            ...
            a.SetTitle(" Data vs MC - "+year);
            a.cd();
            histos_mc[i]->SetYTitle( TString::Format("Candidates / (%g)",((ws->var(variables[i]))->getMax()-(ws->var(variables[i]))->getMin())/n_bins) );
            histos_mc[i]->SetXTitle(x_labels[i]);
            histos_mc[i]->SetStats(0);
            histos_mc[i]->SetStats(0);
            histos_mc[i]->SetTitle(" ");
            ...
            histos_mc[i]->GetYaxis()->SetRangeUser(0.1*histos_mc[i]->GetMinimum(),1.1*histos_mc[i]->GetMaximum());
            histos_mc[i]->Draw("");
            histos_mc[i]->Draw("same");    

            //y axis: maximum and minimum
            if((histos_mc[i]->GetMaximum() > histos_data[i]->GetMaximum())){
                  histos_data[i]->GetYaxis()->SetRangeUser(0.1*histos_mc[i]->GetMinimum(), 1.1*histos_mc[i]->GetMaximum());}
            else if((histos_data[i]->GetMaximum() > histos_mc[i]->GetMaximum())){
                  histos_data[i]->GetYaxis()->SetRangeUser(0.1*histos_data[i]->GetMinimum(), 1.1*histos_data[i]->GetMaximum());}
            ...
            leg->SetBorderSize(0);
            leg->AddEntry(histos_data[i], "Data", "lp");
            leg->AddEntry(histos_mc[i], "MC", "lp");
            leg->SetTextSize(0.03);
            leg->Draw("same");

            a.SaveAs("Data_MC/data_vs_mc_"+names[i]+"_"+year+".pdf");
            a.SaveAs("Data_MC/data_vs_mc_"+names[i]+"_"+year+".gif");
      }
...
TH1D* create_histogram(RooRealVar var, TTree* t, int n, bool mc){
      ...
      TString cut = "(abs(Kp_TRUEID) == 321 && abs(taup_pip0_TRUEID) == 211 && abs(taup_pim0_TRUEID) == 211 && abs(taup_pip1_TRUEID) == 211 && abs(taum_pim0_TRUEID) == 211 && abs(taum_pip0_TRUEID) == 211 && abs(taum_pim1_TRUEID) == 211) && (abs(taup_TRUEID) == 15 && abs(taum_TRUEID) == 15) && (abs(Bp_TRUEID) == 521)";

      if(mc){t->Draw(name_string,cut);}
      else{t->Draw(name_string);}
