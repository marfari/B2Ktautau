...
TH1D* create_histogram(RooRealVar var, TTree* t, int n, bool pass);

using namespace std;

void pass_fail_comparison(){
      ...
      std::vector<TH1D*> histos_pass;
      std::vector<TH1D*> histos_fail;
      int n_bins = 80;
      ...
      for(int i=0; i<n_var; i++){
            histos_pass.push_back(create_histogram((*ws->var(variables[i])), t_mc, n_bins, true));
            histos_fail.push_back(create_histogram((*ws->var(variables[i])), t_mc, n_bins, false));
      ...
      for(int i=0; i<n_var; i++){
            ...
            a.SetTitle(" Pass vs Fail - "+year);
            a.cd();
            histos_pass[i]->SetYTitle( TString::Format("Candidates / (%g)",((ws->var(variables[i]))->getMax()-(ws->var(variables[i]))->getMin())/n_bins) );
            histos_pass[i]->SetXTitle(x_labels[i]);
            histos_pass[i]->SetStats(0);
            histos_pass[i]->SetStats(0);
            histos_pass[i]->SetTitle(" ");

            gStyle->SetOptStat(0);

            //normalization
            histos_fail[i]->Scale(1/histos_fail[i]->Integral());
            histos_pass[i]->Scale(1/histos_pass[i]->Integral());
            histos_pass[i]->GetYaxis()->SetRangeUser(0.1*histos_pass[i]->GetMinimum(),1.1*histos_pass[i]->GetMaximum());
            histos_fail[i]->Draw("HIST");
            histos_pass[i]->Draw("HIST same");    

            //y axis: maximum and minimum
            if((histos_pass[i]->GetMaximum() > histos_fail[i]->GetMaximum())){
                  histos_fail[i]->GetYaxis()->SetRangeUser(0.1*histos_pass[i]->GetMinimum(), 1.1*histos_pass[i]->GetMaximum());}
            else if((histos_fail[i]->GetMaximum() > histos_pass[i]->GetMaximum())){
                  histos_fail[i]->GetYaxis()->SetRangeUser(0.1*histos_fail[i]->GetMinimum(), 1.1*histos_fail[i]->GetMaximum());}

            //--TRATIO--//
            auto rp = new TRatioPlot(histos_fail[i], histos_pass[i], "divsym");
            a.SetTicks(0, 1);
            rp->SetH1DrawOpt("HIST");
            rp->SetH2DrawOpt("HIST");
            rp->Draw();
            rp->GetLowerRefYaxis()->SetTitle("Pass / Fail");
            ...
            leg->SetBorderSize(0);
            leg->AddEntry(histos_pass[i], "Pass", "lp");
            leg->AddEntry(histos_fail[i], "Fail", "lp");
            leg->SetTextSize(0.03);
            leg->Draw("same");

            a.SaveAs("Pass_fail/pass_vs_fail_"+names[i]+"_"+year+".pdf");
            a.SaveAs("Pass_fail/pass_vs_fail_"+names[i]+"_"+year+".gif");
      }
...
TH1D* create_histogram(RooRealVar var, TTree* t, int n, bool pass){
      ...
      TString cut_pass = "(Bp_ConsBp_status == 0) && (abs(Kp_TRUEID) == 321 && abs(taup_pip0_TRUEID) == 211 && abs(taup_pim0_TRUEID) == 211 && abs(taup_pip1_TRUEID) == 211 && abs(taum_pim0_TRUEID) == 211 && abs(taum_pip0_TRUEID) == 211 && abs(taum_pim1_TRUEID) == 211) && (abs(taup_TRUEID) == 15 && abs(taum_TRUEID) == 15) && (abs(Bp_TRUEID) == 521)";
      TString cut_fail = "(Bp_ConsBp_status == 1) &&(abs(Kp_TRUEID) == 321 && abs(taup_pip0_TRUEID) == 211 && abs(taup_pim0_TRUEID) == 211 && abs(taup_pip1_TRUEID) == 211 && abs(taum_pim0_TRUEID) == 211 && abs(taum_pip0_TRUEID) == 211 && abs(taum_pim1_TRUEID) == 211) && (abs(taup_TRUEID) == 15 && abs(taum_TRUEID) == 15) && (abs(Bp_TRUEID) == 521)";

      if(pass){t->Draw(name_string,cut_pass);}
      else{t->Draw(name_string,cut_fail);}
