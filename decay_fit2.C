...
void decay_fit2(int year, TString MC_files, TString RS_DATA_files, TString WS_DATA_files, int species, int component, int line)
{
  TFileCollection* fc;
  if(species == 0)
  { 
...
void minimize( ROOT::Math::XYZPoint BV, int init )
{
  ...
  const Double_t *x_errMin = min->Errors();
  for(int i = 0; i < dimX; i++)
  {
    x_m(i) = xMin[i];
    x_err_m(i) = x_errMin[i];
    for(int j = 0; j < dimX; j++)
    {
...
Double_t chisquare( const Double_t* x_values )
{
  ...
  // ROOT::Math::XYZVector ptau1_m( x(7), x(8), x(9) );
  // ROOT::Math::XYZVector ptau2_m( x(13), x(14), x(15) );
  // ROOT::Math::XYZVector pB_m( x(3), x(4), x(5) );
  // Double_t mB_squared_m = x(6);
  ...
  // Double_t mB_squared_s = pow(Etau1_m + Etau2_m + EK_m, 2) - pB_m.Mag2();

  // Double_t epsilon = 0.1;
  // if( abs(mB_squared_m - mB_squared_s) > epsilon )
  // {
  //   chi2 = pow(10,5);
  // }
...
TVectorD model( TVectorD x )
{
  ...
  // 3-momentum conservation in BV (4):
...
TVectorD x_initial_estimate( TVectorD m ) // Original initialisation for x (based on Anne Keune's thesis)
{
  ...
  // ROOT::Math::XYZPoint RP( m(17), m(18), RPz_t ); // truth-match
