...
void store_vars_in_workspace(RooWorkspace* w, TTree* t, Int_t n_vars, Int_t species);

// D+D-K+ variables
// TString variables[] = {"mass",
...
void createDataset(int year, int species, TString FILES)
{
    ...
    Int_t n_vars = sizeof(variables)/sizeof(variables[0]);
    store_vars_in_workspace(w, t_cut, n_vars, species);
